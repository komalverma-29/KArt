...
const PURCHASABLE_ARTWORK = {
  id: "art1", title: "Sunset", status: "PUBLISHED", forSale: true, availability: "AVAILABLE", price: 250,
};
...
describe("ShopService.isPurchasable", () => {
  ...
  it("is purchasable when Published + For Sale + Available", async () => {
    vi.mocked(ArtworkRepository.findById).mockResolvedValue(PURCHASABLE_ARTWORK as never);
    expect(await ShopService.isPurchasable("art1")).toBe(true);
  });

  it("is not purchasable when Draft", async () => {
    vi.mocked(ArtworkRepository.findById).mockResolvedValue({ ...PURCHASABLE_ARTWORK, status: "DRAFT" } as never);
    expect(await ShopService.isPurchasable("art1")).toBe(false);
  });

  it("is not purchasable when Archived", async () => {
    vi.mocked(ArtworkRepository.findById).mockResolvedValue({ ...PURCHASABLE_ARTWORK, status: "ARCHIVED" } as never);
    expect(await ShopService.isPurchasable("art1")).toBe(false);
  });

  it("is not purchasable when not marked For Sale", async () => {
    vi.mocked(ArtworkRepository.findById).mockResolvedValue({ ...PURCHASABLE_ARTWORK, forSale: false } as never);
    expect(await ShopService.isPurchasable("art1")).toBe(false);
  });

  it("is not purchasable when Sold", async () => {
    vi.mocked(ArtworkRepository.findById).mockResolvedValue({ ...PURCHASABLE_ARTWORK, availability: "SOLD" } as never);
    expect(await ShopService.isPurchasable("art1")).toBe(false);
  });

  it("is not purchasable when Reserved", async () => {
    vi.mocked(ArtworkRepository.findById).mockResolvedValue({ ...PURCHASABLE_ARTWORK, availability: "RESERVED" } as never);
    expect(await ShopService.isPurchasable("art1")).toBe(false);
  });
...
describe("ShopService.listAvailableForSale", () => {
  ...
  it("filters to Published artwork marked For Sale only", async () => {
    ...
    expect(result.map((a: never) => (a as { id: string }).id)).toEqual(["a1"]);
    expect(ArtworkRepository.list).toHaveBeenCalledWith({ status: "PUBLISHED" });
  });
});
...
describe("ShopService.createOrderFromRequest", () => {
  ...
  it("rejects purchase of an unpublished artwork and creates NO order", async () => {
    ...
    await expect(ShopService.createOrderFromRequest(purchaseInput)).rejects.toThrow(ShopServiceError);
  ...
  it("rejects purchase of unavailable (sold) artwork and creates NO order", async () => {
    vi.mocked(ArtworkRepository.findById).mockResolvedValue({ ...PURCHASABLE_ARTWORK, availability: "SOLD" } as never);
    await expect(ShopService.createOrderFromRequest(purchaseInput)).rejects.toThrow(
  ...
  it("rejects purchase of an artwork with no price set, even if otherwise eligible", async () => {
    vi.mocked(ArtworkRepository.findById).mockResolvedValue({ ...PURCHASABLE_ARTWORK, price: null } as never);
    await expect(ShopService.createOrderFromRequest(purchaseInput)).rejects.toThrow(
      "This artwork does not have a price set."
    );
    expect(OrderRepository.createFromPurchase).not.toHaveBeenCalled();
  });
  ...
  it("creates exactly one order (via the atomic repository method) for an eligible artwork", async () => {
    ...
    vi.mocked(OrderRepository.createFromPurchase).mockResolvedValue({ id: "order1", orderNumber: "ORD-X" } as never);

    const order = await ShopService.createOrderFromRequest(purchaseInput);

    expect(order.id).toBe("order1");
    expect(OrderRepository.createFromPurchase).toHaveBeenCalledTimes(1);
    const callArgs = vi.mocked(OrderRepository.createFromPurchase).mock.calls[0][0];
    expect(callArgs.item).toEqual({ artworkId: "art1", artworkTitle: "Sunset", unitPrice: 250, quantity: 1 });
  ...
  it("computes totalAmount as unitPrice × quantity", async () => {
    vi.mocked(ArtworkRepository.findById).mockResolvedValue(PURCHASABLE_ARTWORK as never);
    vi.mocked(OrderRepository.orderNumberExists).mockResolvedValue(false);
    vi.mocked(OrderRepository.createFromPurchase).mockResolvedValue({ id: "order1" } as never);

    await ShopService.createOrderFromRequest({ ...purchaseInput, quantity: 3 });
  ...
  it("generates a unique order number, retrying past a collision", async () => {
    vi.mocked(ArtworkRepository.findById).mockResolvedValue(PURCHASABLE_ARTWORK as never);
    vi.mocked(OrderRepository.orderNumberExists).mockResolvedValueOnce(true).mockResolvedValueOnce(false);
    vi.mocked(OrderRepository.createFromPurchase).mockResolvedValue({ id: "order1" } as never);
  ...
  it("rejects a purchase request for an artwork that no longer exists", async () => {
    vi.mocked(ArtworkRepository.findById).mockResolvedValue(null);
    await expect(ShopService.createOrderFromRequest(purchaseInput)).rejects.toThrow("Artwork not found.");
    expect(OrderRepository.createFromPurchase).not.toHaveBeenCalled();
  });
});